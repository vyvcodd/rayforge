...
def test_shifts_along_scan_direction_at_90_degrees(
    ...
    # Raster generation emits output-space geometry with Y flipped, so at
    # scan_angle=90 the reference (forward) direction is -Y: column 1
    # (top-to-bottom) is forward and untouched.
    assert ops.endpoint(2) == pytest.approx((0.5, 2.0, 0.0), abs=1e-9)
    assert ops.endpoint(3) == pytest.approx((0.5, 0.0, 0.0), abs=1e-9)
    # Columns 0 and 2 (bottom-to-top) run opposite the reference
    # direction, so they're shifted along Y, not X.
    assert ops.endpoint(0) == pytest.approx((0.6, -0.3, 0.0), abs=1e-9)
    assert ops.endpoint(1) == pytest.approx((0.6, 1.7, 0.0), abs=1e-9)
    assert ops.endpoint(4) == pytest.approx((0.4, -0.3, 0.0), abs=1e-9)
    assert ops.endpoint(5) == pytest.approx((0.4, 1.7, 0.0), abs=1e-9)
